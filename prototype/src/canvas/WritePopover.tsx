...
// 撰写对话框（类比 RefinePopover，PRD §4.4：popover 不是 modal）
export function WritePopover({ selectedNodeIds, position, onClose }: Props) {
  ...
  const handleWrite = async () => {
    ...
    try {
      const { node: writtenNode, edges, streamUrl } = await api.write({
        sourceNodeIds: selectedNodeIds,
        writingRequest: writingRequest.trim() || null,
      });
      upsertNode(writtenNode);
      edges.forEach(upsertEdge);

      const msgId = `m_${Math.random().toString(36).slice(2, 11)}`;
      createdNodeId = writtenNode.id;
      createdMsgId = msgId;
      upsertMessage({
        id: msgId,
      ...
      setStreaming(writtenNode.id, 'streaming');
      setActiveNode(writtenNode.id);
      clearSelection();
      onClose();

      // 用统一的 applyStreamEvent 处理流：补齐 error / reasoning_details，并在 done 事件
      // 把乐观 msgId 替换为后端真实 ID（与 conversation 路径 commit eb844bb 对齐），
      // 修复"撰写出错时节点永久卡在 streaming 状态"的根因。
      await api.streamWrite(streamUrl, (evt) => applyStreamEvent(evt, msgId));
      setStreaming(writtenNode.id, 'idle');
    } catch (e) {
      console.error('write failed', e);
      // 网络异常 / fetch 抛错路径：必须翻回 streaming 状态并把消息标记 error，
      // 否则 SSE 流根本没起来，applyStreamEvent 不会被触发，节点永久卡在"撰写中"。
  ...
  return (
    ...
    >
      ...
      >
        <PenLine size={16} strokeWidth={1.8} color={color.accent500} />
      ...
      <textarea
        ...
        onChange={(e) => setWritingRequest(e.target.value)}
        onFocus={() => setFocused(true)}
        onBlur={() => setFocused(false)}
        placeholder="留空表示综合撰写"
        rows={3}
        style={{
          width: '100%',
          fontSize: text.sm,
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: space.s2, marginTop: space.s4 }}>
        ...
        <DialogButton variant="primary" onClick={handleWrite} disabled={submitting}>
          {submitting ? '撰写中…' : '开始撰写'}
        </DialogButton>
      </div>
    </div>
