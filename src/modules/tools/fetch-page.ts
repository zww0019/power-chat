...
import type { FetchPageArgs, FetchPageResult } from '../../types.js';
...
export const fetchPageTool: ToolDefinition<FetchPageArgs, FetchPageResult> = {
  name: 'fetch_page',
  ...
  async execute(args, signal): Promise<ToolExecutionResult<FetchPageResult>> {
    if (signal?.aborted) {
...
async function callTavilyExtract(
...
): Promise<ToolExecutionResult<FetchPageResult>> {
  ...
  try {
    ...
    if (failure) return failure as ToolExecutionResult<FetchPageResult>;
...
function applyContentLimit(raw: string): { content: string; truncated: boolean } {
  if (raw.length <= FETCH_PAGE_MAX_CHARS) return { content: raw, truncated: false };
...
function mockExtractResult(url: string): ToolExecutionResult<FetchPageResult> {
