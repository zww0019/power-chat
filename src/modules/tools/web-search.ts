...
import type { WebSearchArgs, WebSearchResult } from '../../types.js';
...
export const webSearchTool: ToolDefinition<WebSearchArgs, WebSearchResult> = {
  ...
  async execute(args, signal): Promise<ToolExecutionResult<WebSearchResult>> {
    if (signal?.aborted) {
...
async function callTavilySearch(
...
): Promise<ToolExecutionResult<WebSearchResult>> {
  ...
  try {
    ...
    if (failure) return failure as ToolExecutionResult<WebSearchResult>;
...
function mockSearchResult(query: string): ToolExecutionResult<WebSearchResult> {
  return {
    success: true,
    data: {
      results: [
...
export function combineSignalsWithTimeout(
  userSignal: AbortSignal | undefined,
  timeoutMs: number,
...
// 区分超时 / 用户中断 / 网络错误三类
export function classifyTavilyError(e: unknown, userSignal: AbortSignal | undefined): string {
  const err = e as { name?: string; message?: string } | undefined;
  if (err?.name === 'TimeoutError') return 'tavily_timeout';
  if (err?.name === 'AbortError' || userSignal?.aborted) return 'aborted';
...
export async function mapTavilyHttpFailure(res: Response): Promise<ToolExecutionResult<never> | null> {
  if (res.status === 401) return { success: false, error: 'tavily_unauthorized' };
  if (res.status === 429) return { success: false, error: 'tavily_rate_limited' };
  if (!res.ok) {
