...
- HelpDialog（帮助弹窗）：顶部 toolbar 中 `?` 按钮触发，6 个分组（关于 / 核心操作 / 对话操作 / 节点三态 / Agent 联网与思考 / 键盘快捷键），覆盖项目可见功能全集；ESC / 点遮罩 / 点 × 三路关闭；ESC 不分级（与 fullscreen 同时打开会一起关），详见 D025；新增功能须同步该弹窗 JSX
