...
- 状态分三种：**展开态**（画布上原地显示消息列表 + 输入框，宽 360px）、**折叠态**（仅显示标题与最后一条消息预览，宽 200px）、**大屏态（fullscreen）**（覆盖层 Modal 内显示，离开画布 transform 层，详见"大屏对话框"）
...
## 大屏对话框 / 大屏态（Fullscreen Modal）
节点对话的覆盖层 Modal 形态，与展开态/折叠态并列的第三态。
- 触发：用户点击节点 header 的 `⛶` 按钮
- 关闭：ESC / 点遮罩 / 点 Modal header 的 `×`
- 全局唯一：同一时刻最多 1 个节点处于大屏态（与"活跃节点"语义对齐）
- 与节点折叠态的关系：进入大屏时节点同步折叠（避免画布与 Modal 同时展示同一节点的展开内容）；关闭大屏后节点保持折叠态（用户决策 F）
- 不持久化：刷新页面后 Modal 自动关闭，避免恢复到一个用户已经离开的全屏态
- 仅 UI 形态变化，不影响"全局并发 1"流式调度（R018）

## 全局预览（Minimap）
画布右下角固定的缩略视窗，用于俯瞰整个画布并快速跳转。
- 显示所有节点的缩略矩形（按节点位置/折叠态宽高估算等比缩放）+ 当前视口框
- **不显示边（edge）**：小尺寸下视觉收益低
- 单击 minimap 空白区 → 视口居中跳转到对应逻辑坐标
- 在视口框内按住拖拽 → 实时同步画布视口
- 不参与画布 transform，与画布缩放/平移独立
