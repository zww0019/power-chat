...
import { FAKE_SETTINGS_WITH_KEY, FAKE_SETTINGS_NO_KEY, mockTavilyExtractOk, setupToolTestEnv, } from './_helpers';
...
describe('fetch_page 工具单元测试', () => {
    ...
    it('真实路径：构造请求体含 api_key/urls', async () => {
        const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({ results: [{ url: 'https://x.com', raw_content: 'hello' }] }), { status: 200 }));
