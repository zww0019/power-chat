...
export const FAKE_SETTINGS_WITH_KEY: Settings = {
  llmBaseUrl: '',
  llmModel: '',
  llmFastModel: '',
  llmApiKey: '',
...
export const FAKE_SETTINGS_NO_KEY: Settings = {
  ...FAKE_SETTINGS_WITH_KEY,
  tavilyApiKey: '',
};

// 包装一个 Tavily Extract API 风格的成功响应（results[0].raw_content 模式）
export function mockTavilyExtractOk(url: string, rawContent: string): void {
  vi.spyOn(globalThis, 'fetch').mockResolvedValue(
    new Response(
      JSON.stringify({ results: [{ url, raw_content: rawContent }] }),
      { status: 200 },
    ),
  );
}
...
export function setupToolTestEnv(resetMocks: () => void): void {
  beforeEach(() => {
    vi.restoreAllMocks();
