...
describe('web_search 工具单元测试', () => {
  ...
  it('maxResults 缺省时回退 5，且超出 1-10 范围被夹紧', async () => {
    ...
    expect(JSON.parse(fetchSpy.mock.calls[0]![1]!.body as string).max_results).toBe(5);

    fetchSpy.mockClear();
    await webSearchTool.execute({ query: 'a', maxResults: 99 });
    expect(JSON.parse(fetchSpy.mock.calls[0]![1]!.body as string).max_results).toBe(10);

    fetchSpy.mockClear();
    await webSearchTool.execute({ query: 'a', maxResults: 0 });
    expect(JSON.parse(fetchSpy.mock.calls[0]![1]!.body as string).max_results).toBe(1);
  });
