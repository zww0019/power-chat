...
async function createNodeWithContent(content) {
...
describe('refine: 创建提炼任务', () => {
    ...
    it('streamUrl token 一次性使用，第二次拉取返回 token_not_found', async () => {
        ...
        const errorEvt = events2.find((e) => e.type === 'error');
...
describe('refine: INV-4 提炼边的合法性', () => {
    it('每条 refine_input 边的 child 都是 refined 节点', async () => {
        ...
        const refinedIds = new Set(snap.nodes.filter((n) => n.type === 'refined').map((n) => n.id));
