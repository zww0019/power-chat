...
describe('canvas: 首屏快照', () => {
    ...
    it('插入节点后再 GET 返回完整快照', async () => {
        ...
        expect(data.nodes.map((n) => n.id).sort()).toEqual([n1.id, n2.id].sort());
    });
});
describe('canvas: 节点 CRUD', () => {
    ...
    it('PATCH 节点位置后再 GET 返回新位置（旅程3 步骤1）', async () => {
        ...
        const patched = await api(`/api/nodes/${n.id}`, {
            method: 'PATCH',
        ...
        const reloaded = snap.nodes.find((x) => x.id === n.id);
    ...
    it('删除节点返回 204，再 GET 不再出现', async () => {
        ...
        const snap = await api('/api/canvas');
        expect(snap.nodes.find((x) => x.id === n.id)).toBeUndefined();
    });
    it('删除节点级联删除入/出边，子孙节点保留（INV-5, Q2-3 选 B）', async () => {
        ...
        expect(res.status).toBe(204);
        const snap = await api('/api/canvas');
        expect(snap.nodes.find((x) => x.id === A.id)).toBeUndefined();
        expect(snap.nodes.find((x) => x.id === B.id)).toBeDefined(); // 子节点保留
        // 但 A→B 的边断开
        expect(snap.edges.find((e) => e.parentNodeId === A.id || e.childNodeId === A.id)).toBeUndefined();
    });
...
describe('canvas: 边删除', () => {
    it('DELETE /api/edges/:id 删除单条边返回 204，节点保留', async () => {
        ...
        expect(res.status).toBe(204);
        const snap = await api('/api/canvas');
        expect(snap.edges.find((e) => e.id === edge.id)).toBeUndefined();
        // 边删除不影响两端节点
        expect(snap.nodes.find((x) => x.id === A.id)).toBeDefined();
        expect(snap.nodes.find((x) => x.id === B.id)).toBeDefined();
    });
