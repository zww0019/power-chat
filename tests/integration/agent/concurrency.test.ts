...
async function startLoopingAgentStream(
  nodeId: string,
): Promise<Promise<Array<{ type: string; [k: string]: unknown }>>> {
  ...
  while (Date.now() - start < 2000) {
    const info = await fetch(`${BASE_URL}/api/__test__/streaming-info`).then((r) => r.json()) as {
      streamingNodeIds: string[];
    };
    if (info.streamingNodeIds.includes(nodeId)) return ssePromise;
...
describe('agent: 全局并发守卫', () => {
  ...
  it.skip('A 节点流式中，B 节点 send 直接返回 409 streaming_busy', async () => {
    ...
    const body = (await bRes.json()) as { error: string; streamingNodeIds?: string[] };
