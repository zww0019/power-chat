...
async function getAssistantMessage(nodeId) {
...
function expectAgentFlowEvents(events) {
...
function findActionEvent(events) {
    return events.find((e) => e.type === 'agent_action');
}
describe('agent: ReAct loop 端到端 - native_tools 模式', () => {
    it('用户用"搜一下"动作动词触发时，SSE 流包含 agent_action + agent_observation + done', async () => {
        ...
        const observation = events.find((e) => e.type === 'agent_observation');
    ...
    it('user 含 URL 时触发 fetch_page 工具', async () => {
        ...
        const action = events.find((e) => e.type === 'agent_action');
