 ...
 */
export async function sendMessage(nodeId, content) {
 ...
 */
export async function createBranchedPair(seedContent = '触发分支') {
    const parent = await createNode();
    const events = await sendMessage(parent.id, seedContent);
    const done = events.find((e) => e.type === 'done');
    if (!done)
        throw new Error('no done event from sendMessage stream');
    const branch = await api('/api/nodes/branch', {
        method: 'POST',
 ...
 */
export async function consumeSSE(path, init) {
